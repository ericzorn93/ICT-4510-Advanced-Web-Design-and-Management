/* Eric Zorn: Module 3 (Forms) with JavaScript and jQuery ICT 4510  10/6/17*/
...
/*
    ...
    Vanilla JavaScript. I have added the NEW code to the BOTTOM of this document after the coe that has been commented out.
    */
...
/* Module 5 Homework Assignment Code 10.6.17 */


function getValues() {
    // let values = [];
    //
    // values.push($("#firstName").val());
    // values.push($("#lastName").val());
    // values.push($("#email").val());
    // values.push($("#phoneNumber").val());
    // values.push($("#emailRadio").val());
    // values.push($("#phoneRadio").val());
    ...
    return {
        ...
        emailRadio: $("#emailRadio").val(),
        phoneRadio: $("#phoneRadio").val()
    }
}
...
$(document).ready(function () {
    ...
    $form.validate({
        ...
        submitHandler: function () {
            ...
            let inputVals = getValues();

            $('input[name=contactPreference]:checked', '#myForm').val();

            $.post("process.php", inputVals, function (data) {
                for (key in inputVals) {
                    const content = $("#formValues").textContent = '<li>' + inputVals[key] + '</li>';
                    $("#myJumbotron").append(content).show(3000);
                }
            });
        ...
        messages: {
            firstName: {
                required: "You must enter your first name in this form!"
            },
            lastName: {
                required: "You must enter your last name in this form!"
            },
            email: {
                required: "You must enter your email in this form!"
            },
            phone: {
                required: "You must enter your phone number in this form!"
            }
        }
    });
});